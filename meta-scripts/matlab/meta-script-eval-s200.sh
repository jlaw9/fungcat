...
#"-A localplus -A sinksourceplus --max-iters 20 --alpha 0.95 --eps 0.0" 
...
#for curr_idx in ${iter_list[@]}; do
    ...
    exp_name="ml-${th}${ev_codes}-50-1000${eval_ev_codes}${string}-${h}"
    ...
    #    echo "skipping $log_file"
        ...
        #    """
        cmd="""time $python -u src/algorithms/gain_matlab/eval_leave_one_species_out.py \
            --version $version \
            --exp-name ${exp_name} \
        ...
        # setup the qsub file
        # set the memory limit to try to make sure each job gets its own node
        # doesn't seem to do anything...
        #echo "#PBS -l nodes=1:ppn=$cores,walltime=$walltime,mem=70000000KB" > $qsub_file
        ...
        # the matlab engine is currently only installed on python2
