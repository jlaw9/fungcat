...
#            unranked_nodes = self.rank_nodes.copy()
        ...
        while len(R) > k:
            ...
            if self.verbose:
                print("\t\t%0.2f sec to update bounds. delta_N: %0.4f" % (update_time, delta_N))
    ...
    def update_N_B_F_matrix(self, N, B, E, F):
        ...
        new_neighbors = set([Fl[i] for i in self.P[El][:,Fl].sum(axis=0).nonzero()[1]])
        ...
        B = self.get_boundary_nodes_matrix(list(B), list(F))
    ...
    def get_boundary_nodes_matrix(self, N, F):
        ...
        B = set([N[i] for i in self.P[N][:,F].sum(axis=1).nonzero()[0]])
    ...
    def initialize_sets(self):
        ...
        B = self.get_boundary_nodes_matrix(list(N), list(F))
