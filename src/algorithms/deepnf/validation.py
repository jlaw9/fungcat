...
"""
goterms is a list of labels
"""
def cross_validation(X, y, goterms, n_trials=5, ker='rbf', bootstraps=None, predicton=True):
    ...
    for jj in range(0, n_trials):
        ...
        clf.fit(K_rbf[gamma_opt], y)
        prediction = clf.predict_proba(K_rbf[gamma_opt])
    perf = dict()
    # perf['pr_micro'] = pr_micro
    # perf['pr_macro'] = pr_macro
    # perf['fmax'] = fmax
    # perf['acc'] = acc
    perf['pr_goterms'] = prediction
